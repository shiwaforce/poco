#!/usr/bin/env python
"""PROCO - project compose.

Usage:
  proco [options] <command> [<args>...]


Options:
  --version         Print version of PROCO
...
The available proco commands are:
"""
import inspect
import importlib
import pkgutil
...
class Proco(object):
    ...
    def __init__(self, home_dir=os.path.join(os.path.expanduser(path='~'), '.proco'),
                 argv=sys.argv[1:]):
    ...
    def get_full_doc(self):
        ...
        for sub_cmd in self.command_classes.keys():
            if sub_cmd is None:
                [Proco.build_command(commands=commands, cls=cls) for cls in self.command_classes[None]]
                continue
            sub = "  " + sub_cmd + " [<subcommand>]"
            doc += sub + (42-len(sub)) * " " + "See 'proco help " + sub_cmd + "' for more.\n"
        doc += "".join(commands) + "\n" + END_STRING
        return doc
    ...
    def command_interpreter(self, command, argv):
        ...
        if command in self.command_classes.keys():
            ...
            if args is None:
                ColorPrint.exit_after_print_messages("%r is not a proco command. See 'proco help'." % command)
        return args
    ...
    def get_args(self, command, classes, argv):
        for cls in classes:
            ...
            if argv[0] in cmd:
                ...
                return docopt(Proco.build_command_help(cls), argv=[command] + argv if command is not None else argv)
    ...
    def build_sub_commands_help(sub_command, classes):
        doc = "Proco " + sub_command + " commands\n\nUsage:\n"
        commands = []
        [Proco.build_command(commands=commands, cls=cls) for cls in classes]
        return doc + "".join(commands)

    def collect_commands(self):
        try:
            command_packages = importlib.import_module('proco.commands', package='proco')
            for importer, modname, ispkg in pkgutil.iter_modules(command_packages.__path__):
                if not ispkg:
                    mod = importlib.import_module('proco.commands.' + modname, 'proco')
