...
class Init(AbstractCommand):
    ...
    description = "Run: 'proco init nginx' or 'proco project init nginx' to initialize proco project, proco.yml " \
    ...
    def execute(self):
        if StateHolder.repository is not None:
            target_file = os.path.join(ProjectUtils.get_target_dir(StateHolder.catalog_element),
                                       StateHolder.catalog_element.get('file', 'proco.yml'))
            if not os.path.exists(target_file):
                Init.fix_file(target_file)
        else:
            file = FileUtils.get_backward_compatible_proco_file(directory=os.getcwd())
            if file is None:
                Init.fix_file(os.path.join(os.getcwd(), 'proco.yml'))
    ...
    def fix_file(target_file):
        if not os.path.exists(target_file):
            src_file = os.path.join(os.path.dirname(__file__), '../services/resources/proco.yml')
