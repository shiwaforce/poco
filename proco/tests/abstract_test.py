...
class AbstractTestSuite(unittest.TestCase):
    ...
    REMOTE_CONFIG = {
      'default': {
        'repositoryType': 'git',
        'file': 'proco-catalog.yml',
        'server': 'https://github.com/shiwaforce/proco-example.git',
        'branch': 'master'
        }
    }
    ...
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix='proco-home')
        self.config_file = os.path.join(self.tmpdir, 'config')
        self.proco_file = os.path.join(self.tmpdir, '.proco')
        self.local_stack_list = os.path.join(self.tmpdir, 'proco-catalog.yml')
    ...
    def init_proco_config(self, params):
        with open(self.proco_file, 'w+') as stream:
    ...
    def run_proco_command(self, *args):
        runnable = proco.Proco(home_dir=self.tmpdir, argv=list(args))
        runnable.start_flow()
    ...
    def init_proco_file(self):
        proco_file = dict()
        proco_file['plan'] = dict()
        proco_file['plan']['default'] = 'docker-compose.yaml'
        with open(os.path.join(self.ws_dir, 'proco.yaml'), 'w+') as stream:
            yaml.dump(data=proco_file, stream=stream, default_flow_style=False,
                      default_style='', indent=4)
