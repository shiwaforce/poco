...
import os
import yaml
import proco.proco as proco
from .abstract_test import AbstractTestSuite
from proco.services.file_utils import FileUtils
from proco.services.cta_utils import CTAUtils


class ProcoTestSuite(AbstractTestSuite):

    def test_without_command(self):
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command()
            self.assertIsNotNone(context.exception)
        self.assertIn(proco.__doc__.strip(), out.getvalue().strip())
        self.assertIn(proco.END_STRING.strip(), out.getvalue().strip())

    def test_version(self):
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("--version")
            self.assertIsNotNone(context.exception)
        self.assertIn(proco.__version__, out.getvalue().strip())

    def test_help_command(self):
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("help")
            self.assertIsNotNone(context.exception)
        self.assertIn(proco.__doc__.strip(), out.getvalue().strip())
    ...
    def test_help_command_with_proco_file(self):
    ...
    def test_subcommand_help(self):
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("repo")
            self.assertIsNotNone(context.exception)
        output = out.getvalue().strip()
        self.assertIn("Proco repo commands\n", output)
        self.assertIn("Usage:", output)

    def test_wrong_parameters(self):
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("notexistcommand")
            self.assertIsNotNone(context.exception)
        self.assertIn("is not a proco command. See 'proco help'.", out.getvalue().strip())

    def test_config_without_config(self):
        with self.captured_output() as (out, err):
            self.run_proco_command("repo", "ls")
    ...
    def test_config_with_config_and_options(self):
        self.init_with_local_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("--offline", "--always-update", "repo", "ls")
        out_string = out.getvalue().strip()
        self.assertIn("Actual config\n-------------\n", out_string)
        self.assertIn("Mode: developer\nOffline: True\nAlways update: True", out_string)
        self.assertIn("Working directory: " + str(self.ws_dir), out_string)
    ...
    def test_catalog_without_catalog(self):
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("catalog")
            self.assertIsNotNone(context.exception)
    ...
    def test_catalog_with_empty_catalog(self):
        ...
        with self.captured_output() as (out, err):
            self.run_proco_command("catalog")
        self.assertEqual(0, len(err.getvalue()))
        self.assertIn("Project catalog is empty. You can add projects with 'proco repo add' command",
                      out.getvalue().strip())
    ...
    def test_catalog(self):
        ...
        with self.captured_output() as (out, err):
            self.run_proco_command("project", "ls")
        self.assertEqual(0, len(err.getvalue()))
    ...
    def test_catalog_with_param_that_not_exists(self):
        ...
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("catalog", "asd")
            self.assertIsNotNone(context.exception)
        self.assertEqual(0, len(err.getvalue()))
        catalog = out.getvalue().strip()
        ...
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("project", "ls", "asd")
            self.assertIsNotNone(context.exception)
        self.assertEqual(0, len(err.getvalue()))
        self.assertEqual(catalog, out.getvalue().strip())
    ...
    def test_catalog_with_param(self):
        self.init_with_remote_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("catalog", "default")
        self.assertEqual(0, len(err.getvalue()))
        catalog = out.getvalue().strip()
        self.assertIn("Available projects:", catalog)
        with self.captured_output() as (out, err):
    ...
    def test_add_modify_and_remove_config(self):
        ...
        with self.captured_output() as (out, err):
            self.run_proco_command("repo", "add", "test", "https://github.com/shiwaforce/proco-example",
                                   "master", "proco-catalog.yaml")
        self.assertEqual(0, len(err.getvalue()))
        data = dict()
        data["test"] = dict()
        data["test"]["branch"] = "master"
        data["test"]["file"] = "proco-catalog.yaml"
        data["test"]["repositoryType"] = "git"
        data["test"]["server"] = "https://github.com/shiwaforce/proco-example"
        ...
        with self.captured_output() as (out, err):
            self.run_proco_command("repo", "modify", "test", "https://github.com/shiwaforce/procok.git",
                                   "master", "test2.yml")
    ...
    def test_add_modify_and_remove_config_github(self):
        self.init_with_remote_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("github", "add", "test", "thisistoken", "http://test/test")
        self.assertEqual(0, len(err.getvalue()))
        data = dict()
        data["test"] = dict()
        ...
        with self.captured_output() as (out, err):
            self.run_proco_command("github", "modify", "test", "user/pass")
    ...
    def test_add_modify_and_remove_config_gitlab(self):
        self.init_with_remote_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("gitlab", "add", "test", "xxxxxx",
        ...
        with self.captured_output() as (out, err):
            self.run_proco_command("gitlab", "modify", "test", "anothertoken")
    ...
    def test_branches_with_local_config(self):
        self.init_with_local_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("repo", "branches")
    ...
    def test_switch_branch_with_local_config(self):
        self.init_with_local_catalog()
        with self.assertRaises(SystemExit) as context:
            self.run_proco_command("repo", "branch", "master")
    ...
    def test_push_with_local_config(self):
        self.init_with_local_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("repo", "push")
        self.assertEqual(0, len(err.getvalue().strip()))
    ...
    def test_failed_add(self):
        ...
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("project", "add", test_dir)
            self.assertIsNotNone(context.exception)
        self.assertIn("Target directory or parents are not a valid git repository", out.getvalue().strip())

    def test_failed_add_without_proco_file(self):
        self.init_with_local_catalog()
        test_dir = os.path.join(self.tmpdir, "test-directory")
        os.makedirs(test_dir)
        git.Repo.clone_from(url=AbstractTestSuite.STACK_LIST_SAMPLE['nginx']['git'], to_path=test_dir)
        os.remove(os.path.join(test_dir, 'nginx', 'proco.yml'))
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("project", "add", test_dir)
            self.assertIsNotNone(context.exception)
        self.assertIn("Directory not contains Proco file!", out.getvalue().strip())
    ...
    def test_add_and_remove(self):
        ...
        os.makedirs(test_dir)
        git.Repo.clone_from(url=AbstractTestSuite.STACK_LIST_SAMPLE['nginx']['git'], to_path=test_dir)
        os.rename(os.path.join(test_dir, 'nginx', 'proco.yml'), os.path.join(test_dir, 'proco.yml'))
        with self.captured_output() as (out, err):
            self.run_proco_command("project", "add", test_dir)
        self.assertEqual(0, len(err.getvalue().strip()))
        ...
        self.clean_states()
        with self.captured_output() as (out, err):
            self.run_proco_command("project", "remove", "test-directory")
        self.assertEqual(0, len(err.getvalue().strip()))
    ...
    def test_init_without_catalog(self):
        self.init_with_local_catalog()
        self.assertIsNone(FileUtils.get_file_with_extension('proco', directory=self.ws_dir))
        self.assertIsNone(FileUtils.get_file_with_extension('docker-compose', directory=self.ws_dir))
        with self.captured_output() as (out, err):
            self.run_proco_command("project", "init")
        self.assertEqual(0, len(err.getvalue().strip()))
        self.assertIsNotNone(FileUtils.get_file_with_extension('proco', directory=self.ws_dir))
        self.assertIsNotNone(FileUtils.get_file_with_extension('docker-compose', directory=self.ws_dir))

    def test_checkout(self):
        self.init_with_remote_catalog()
        base_dir = os.path.join(self.ws_dir, 'proco-example')
        directory = os.path.join(base_dir, 'nginx')
        self.assertFalse(os.path.exists(directory))
        with self.captured_output() as (out, err):
            self.run_proco_command("checkout", "nginx")
    ...
    def test_plan_list_with_not_exists_project(self):
        ...
        with self.captured_output() as (out, err):
            with self.assertRaises(SystemExit) as context:
                self.run_proco_command("plan", "ls")
            self.assertIsNotNone(context.exception)
        self.assertIn("Proco file not found in directory:", out.getvalue())

    def test_plan_list(self):
        self.init_with_remote_catalog()
        with self.captured_output() as (out, err):
            self.run_proco_command("plan", "ls", "nginx")
        self.assertEqual(0, len(err.getvalue()))
