...
class ProjectUtils:
    ...
    def get_file_from_project(file_element, repo_handler):
        if file_element is not None:
            ...
            file = FileUtils.get_backward_compatible_proco_file(directory=repo_handler.target_dir, silent=True)
            return file if file is not None else os.path.join(repo_handler.target_dir, "proco.yaml")
