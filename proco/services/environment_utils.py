...
class EnvironmentUtils:
    ...
    def check_version(version):
        ...
        # check pip
        p = Popen("pip install proco==", stdout=PIPE, stderr=PIPE, shell=True)
        out, err = p.communicate()
        if not len(err) == 0:
        ...
        if version < newest_version:
            ColorPrint.print_warning("New version of proco is available (%r). \n "
                                     "Please upgrade with: pip install -U proco" % newest_version)
