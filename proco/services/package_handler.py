...
class PackageHandler(object):
    ...
    def unpack(self):
        ...
        for file in next(os.walk(os.getcwd()))[2]:
            if file.endswith(".procok"):
                proco_file = file
                tar = tarfile.open(file)
                tar.extractall()
                tar.close()
                break
        if proco_file is None:
            ColorPrint.exit_after_print_messages(message=".proco file not exists in current directory")

        cmd = list()
        cmd.append("docker")
        cmd.append("load")
        cmd.append("-i")
        cmd.append(proco_file.rstrip("proco") + "tar")
