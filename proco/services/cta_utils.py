...
class CTAUtils:
    ...
    CTA_STRINGS = {
        "default": "'proco repo add sample https://github.com/shiwaforce/proco-example'\n"
                   "Run if you want sample projects.",
        "have_file": "'proco init'\nYou have some local files for virtualize your project. "
    ...
    def get_cta():
        res = ""
        if CTAUtils.one_of_local_files_exits(files=['proco.yml', 'proco.yaml']):
            res = CTAUtils.CTA_STRINGS['have_all']
        elif not CTAUtils.catalog_exists() and CTAUtils.one_of_local_files_exits(
                files=['docker-compose.yml', 'docker-compose.yaml', '.proco', 'docker']) \
                and not CTAUtils.one_of_local_files_exits(files=['proco.yml', 'proco.yaml']):
