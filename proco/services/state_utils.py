...
class StateUtils:
    ...
    def prepare(prepareable=None):
        ...
        if prepareable is "compose_handler":
            StateHolder.compose_handler = ComposeHandler(StateHolder.proco_file)
        StateHolder.process_extra_args()

    @staticmethod
    def prepare_config():
        if StateHolder.global_config_file is None:
            StateHolder.global_config_file = os.path.join(StateHolder.home_dir, '.proco')
    ...
    def prepare_project_file():
        if StateHolder.repository is None:
            StateHolder.proco_file = FileUtils.get_backward_compatible_proco_file(directory=os.getcwd())
    ...
    def calculate_name_and_work_dir():
        ...
        if arg is None:  # if empty
            ...
            local_project_file = FileUtils.get_backward_compatible_proco_file(silent=True)
