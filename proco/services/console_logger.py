...
class Doc:
    ...
    If default section is empty, the proco-catalog.yml file must be exists in config directory

    repositoryType (optional):  git | svn | file
    url (optional): must be a valid GIT or SVN url
    file (optional):    catalog file path in the repository or local filesystem - default : proco-catalog.yml
    ...
    PROCO_CATALOG = """Pocok catalog.

        YAML format.

        Generate a sample proco file for project, please run:
            proco init
        ...
        Configuration:
            ...
            branch (optional): branch name - default : master
            file(optional): path to the proco file. - Default : proco.yml
                If you not define repository, it will relative to config file
                If the path is a directory, then will extended the path the default filename : proco.yml
