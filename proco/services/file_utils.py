...
class FileUtils:
    ...
    # TODO remove later
    @staticmethod
    def get_backward_compatible_proco_file(directory=None, throw_exception=False, silent=False):
        ...
        if file is not None:
            if not silent:
                ColorPrint.print_info("Your configuration file (" + file +
                                      ") is deprecated! Use 'proco.yaml/yml' instead.")
            return file
        if throw_exception:
            ColorPrint.exit_after_print_messages("Directory not contains Proco file!")
